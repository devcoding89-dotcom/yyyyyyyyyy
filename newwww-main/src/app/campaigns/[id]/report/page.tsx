...
export default function CampaignReportPage() {
  ...
  const campaignQuery = useMemoSupabaseDoc(user && id ? { tableName: "campaigns", id: id, filters: [{ column: "user_id", operator: "eq", value: user.id }] } : null, [user, id]);

  const { data: campaign, isLoading: campaignLoading } = useDoc(campaignQuery);

  const logsQuery = useMemoSupabaseCollection(user && id ? {
